...
@Component({
	...
	templateUrl: './ng-docs.component.html',
	styleUrls: ['./ng-docs.style.scss'],
})
export class DsNgDocsComponent {
	/**
	...
	set directive(directiveName: string) {
		this.ngDocs = docs[directiveName];
	...
	private _findInputConfigProperty(
		configNgDocs: ClassDesc,
		input: InputDesc,
	): PropertyDesc {
		return configNgDocs.properties.filter(prop => prop.name === input.name)[0];
	}
}
